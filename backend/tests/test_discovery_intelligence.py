...
from app.services.discovery_intelligence_service import DEVICE_FAMILIES,PIPELINE,configuration_checksum,confidence,identify,merge_hostnames,review_status,services_from_ports
...
def test_confidence_is_explainable_and_capped():
    result=confidence(["ping_response","mac_address","vendor_match","hostname_match","snmp","ad_match","cmdb_match","lldp","service_fingerprint","operating_system"]);assert result["score"]==100 and sum(x["weight"] for x in result["contributions"])==110;assert confidence(["ping_response","ping_response"])["score"]==15
