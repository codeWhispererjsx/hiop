...
class DiscoveryIntelligenceService:
    ...
    def ingest(self,job,observation):
        ...
        item.primary_hostname=names["primary"];item.mac_address=observation.get("mac_address");item.vendor=observation.get("vendor");item.device_type=identified["device_family"];item.classification=identified["classification"];item.operating_system=observation.get("operating_system")
        ...
        for raw in observation.get("evidence",[]):
            ...
            evidence.append(key)
        score=confidence(evidence);item.confidence_score=score["score"];item.confidence_explanation=json.dumps(score["contributions"]);item.review_status=review_status(item.confidence_score)
