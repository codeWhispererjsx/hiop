...
def scoped_alert(db,alert_id,organization_id,property_id=None):
    query=db.query(Alert).join(Device,Alert.device_id==Device.id).join(Property,Device.property_id==Property.id).filter(Alert.id==alert_id,Property.organization_id==organization_id)
...
def acknowledge(alert_id:UUID,db:Session=Depends(get_db),user:User=Depends(operator),organization_id=Depends(organization_context),property_id=Depends(property_context)):
    row=scoped_alert(db,alert_id,organization_id,property_id)
...
def resolve(alert_id:UUID,payload:Resolution,db:Session=Depends(get_db),user:User=Depends(operator),organization_id=Depends(organization_context),property_id=Depends(property_context)):
    row=scoped_alert(db,alert_id,organization_id,property_id)
...
@router.get("/events")
def events(limit:int=Query(200,ge=1,le=1000),db:Session=Depends(get_db),_=Depends(reader),organization_id=Depends(organization_context),property_id=Depends(property_context)):
    query=db.query(MonitoringEvent).join(Device,MonitoringEvent.device_id==Device.id).join(Property,Device.property_id==Property.id).filter(Property.organization_id==organization_id)
    if property_id:query=query.filter(Device.property_id==property_id)
