...
def topology_graph(
    ...
    return V3ATopologyService(db).graph(search)
...
def relationship_details(relationship_id: UUID, db: Session = Depends(get_db), _=Depends(reader)):
    return V3ATopologyService(db).relationship(relationship_id)


@router.get("/devices/{device_id}/neighbors")
def device_neighbors(device_id: UUID, db: Session = Depends(get_db), _=Depends(reader)):
    return V3ATopologyService(db).device_neighbors(device_id)


@router.get("/stats")
def topology_stats(db: Session = Depends(get_db), _=Depends(reader)):
    return V3ATopologyService(db).stats()
