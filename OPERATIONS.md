...
After sync, run matching with a bounded limit. Investigate conflict counts before bulk action. If a resolution returns stale-source or stale-target conflict, rerun matching and review again. Use the `ad-reconciliation` report and audit log for operational evidence. Never treat a disabled/missing AD object as authorization to disable or retire a HIOP record.
