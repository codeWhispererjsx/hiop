...
function InviteUser({close}:{close:()=>void}){
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault();setBusy(true);setMessage("");try{const result=await endpoints.inviteOrganizationUser(email,role,role==="admin"?undefined:property||undefined);if(result.delivery_status!=="sent")throw new Error("Invitation email could not be sent. Please try again.");setMessage("Invitation sent. Ask your teammate to check their inbox and spam folder.");setEmail("")}catch(reason){setMessage(reason instanceof Error?reason.message:"Unable to create invitation.")}finally{setBusy(false)}};
  return <Modal title="Invite organization user" onClose={()=>{if(!busy)close()}}><form className="settings-form-grid" onSubmit={submit}><p className="form-help">The recipient receives a single-use link that expires after 72 hours.</p><label className="field-label">Work email<input type="email" value={email} onChange={event=>setEmail(event.target.value)} required/></label><label className="field-label">Role<select value={role} onChange={event=>setRole(event.target.value)}><option value="viewer">Viewer</option><option value="technician">Technician</option><option value="admin">Organization administrator</option></select></label><label className="field-label">Property access<select required={role!=="admin"} disabled={role==="admin"} value={role==="admin"?"":property} onChange={event=>setProperty(event.target.value)}><option value="">{role==="admin"?"All properties (administrator)":"Choose a property"}</option>{properties.data?.properties.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{message&&<p role="status">{message}</p>}<div className="modal-actions"><button type="button" className="secondary-action" onClick={close}>Cancel</button><button className="primary-action" disabled={busy}>{busy?"Sending…":"Send invitation"}</button></div></form></Modal>
}
