...
function InviteUser({close}:{close:()=>void}){
  const properties=useRequest(endpoints.propertyContext,[]);const [email,setEmail]=useState("");const [role,setRole]=useState("viewer");const [property,setProperty]=useState("");const [message,setMessage]=useState("");const [inviteLink,setInviteLink]=useState("");const [busy,setBusy]=useState(false);
  const submit=async(event:FormEvent)=>{event.preventDefault();setBusy(true);setMessage("");setInviteLink("");try{const result=await endpoints.inviteOrganizationUser(email,role,property||undefined);if(result.test_token){setInviteLink(`${window.location.origin}/accept-invitation?token=${encodeURIComponent(result.test_token)}`);setMessage("Invitation created. Copy and share the link below while email delivery is not configured.")}else{setMessage(`Invitation created. Delivery: ${result.delivery_status}.`)}setEmail("")}catch(reason){setMessage(reason instanceof Error?reason.message:"Unable to create invitation.")}finally{setBusy(false)}};
  const copy=()=>void navigator.clipboard.writeText(inviteLink).then(()=>setMessage("Invitation link copied."));
  return <Modal title="Invite organization user" onClose={close}><form className="form-grid" onSubmit={submit}><p className="form-help">Create an account invitation. In testing, HIOP also shows a copyable link if email delivery is not configured.</p><label className="field-label">Work email<input type="email" value={email} onChange={event=>setEmail(event.target.value)} required/></label><label className="field-label">Role<select value={role} onChange={event=>setRole(event.target.value)}><option value="viewer">Viewer</option><option value="technician">Technician</option><option value="admin">Organization administrator</option></select></label><label className="field-label">Property access<select value={property} onChange={event=>setProperty(event.target.value)}><option value="">Organization-wide</option>{properties.data?.properties.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{message&&<p role="status" className="inline-notice">{message}</p>}{inviteLink&&<div className="settings-note"><strong>Invitation link</strong><code style={{display:"block",overflowWrap:"anywhere"}}>{inviteLink}</code><button type="button" className="secondary-action" onClick={copy}>Copy invitation link</button></div>}<div className="modal-actions"><button type="button" className="secondary-action" onClick={close}>Cancel</button><button className="primary-action" disabled={busy}>{busy?"Sending...":"Create invitation"}</button></div></form></Modal>
}
