...
export default function AdministrationPage(){
  ...
  return <DashboardLayout><PageTitle eyebrow="V3 operational access checkpoint" title={mode==="roles"?"Roles & access":mode==="audit"?"Audit log":"Administration"} copy={mode==="roles"?"A small, explicit role model for hotel IT operations.":mode==="audit"?"Trace real administrative actions to authenticated users.":"Choose a focused administration workspace instead of searching through one large settings page."}/>
    ...
    <section className="admin-access-grid">
      ...
      <article className="panel admin-entry"><Icon name="lock"/><div><h2>Roles & access</h2><p>Review the four roles and their backend-enforced permissions.</p></div><Link className="secondary-action" to="/administration/roles">Review roles</Link></article>
    ...
    {mode==="roles"&&<section className="panel admin-roles"><header><div><span>Role model</span><h2>Roles and permissions</h2></div><strong>4 baseline roles</strong></header>{roles.loading||roles.error?<Feedback loading={roles.loading} error={roles.error}/>:<div>{roles.data?.map(role=><article key={role.key}><div><strong>{role.name}</strong><span>{role.description}</span></div><ul>{role.permissions.map(permission=><li key={permission}>{permission.replaceAll("_"," ").replaceAll("."," · ")}</li>)}</ul></article>)}</div>}</section>}
