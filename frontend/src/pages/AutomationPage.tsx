...
import type {AutomationApproval,AutomationCorrelationGroup,AutomationDeadLetter,AutomationEvent,AutomationReportSummary,AutomationRun,AutomationSchedule,AutomationTrigger,AutomationVersion,AutomationWorkflow} from "../lib/types";
...
export default function AutomationPage(){
  ...
  const [correlations,setCorrelations]=useState<AutomationCorrelationGroup[]>([]);
  const [deadLetters,setDeadLetters]=useState<AutomationDeadLetter[]>([]);
  const [summary,setSummary]=useState<AutomationReportSummary>();
  ...
  const load=async()=>{
    try{
      const [w,r,a,t,s,h,e,c,d,m]=await Promise.all([
        ...
        endpoints.automationEvents(),endpoints.automationCorrelationGroups(),endpoints.automationDeadLetters(),
        endpoints.automationReportSummary(),
      ]);
      ...
      setCorrelations(c.items);setDeadLetters(d.items);setSummary(m);setError("");
  ...
  useEffect(()=>{
    ...
    void Promise.all([
      ...
      endpoints.automationEvents(),endpoints.automationCorrelationGroups(),endpoints.automationDeadLetters(),
      endpoints.automationReportSummary(),
    ]).then(([w,r,a,t,s,h,e,c,d,m])=>{
      if(!active)return;
      setWorkflows(w.items);setRuns(r.items);setApprovals(a.items);setTriggers(t.items);
      setSchedules(s.items);setSchedulerRunning(h.scheduler_running);setEvents(e.items);
      setCorrelations(c.items);setDeadLetters(d.items);setSummary(m);
  ...
  const createSchedule=async(e:FormEvent<HTMLFormElement>)=>{
    ...
    try{
      const scheduleType=String(data.get("schedule_type"));const calendar=scheduleType!=="interval";
      await endpoints.createAutomationSchedule({workflow_id:selected.id,workflow_version_id:selectedVersion.id,property_id:selected.property_id,name:String(data.get("name")),schedule_type:scheduleType,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||"UTC",interval_minutes:scheduleType==="interval"?Number(data.get("interval")):null,preferred_time:calendar?String(data.get("preferred_time")):null,day_of_week:scheduleType==="weekly"?Number(data.get("day")):null,day_of_month:scheduleType==="monthly"?Number(data.get("day")):null,jitter_seconds:30,enabled:false,approval_mode:"use_workflow_policy",maintenance_behavior:"suppress",blackout_behavior:"suppress"});
      form.reset();await load();
  ...
  return <DashboardLayout onLiveEvent={event=>{if((event.type??event.event)?.startsWith("automation_"))void load()}}>
    ...
    <section className="stats-grid">
      ...
      <StatCard label="Event triggers" value={summary?.active_triggers??triggers.length} detail={`${summary?.suppressed??0} suppressed`} icon="network"/>
    ...
    {selected&&selectedVersion&&<section className="hospitality-panel"><div className="panel-heading"><div><h2>Safe orchestration</h2><span>New schedules and triggers are disabled or approval-gated by default.</span></div></div><div className="hospitality-grid"><form className="hospitality-card topology-form" onSubmit={createSchedule}><strong>Schedule builder</strong><label>Name<input name="name" required maxLength={120}/></label><label>Recurrence<select name="schedule_type"><option value="interval">Interval</option><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></label><label>Interval minutes<input name="interval" type="number" min={5} max={10080} defaultValue={60}/></label><label>Calendar time<input name="preferred_time" type="time" defaultValue="08:00"/></label><label>Weekday (0–6) or day (1–28)<input name="day" type="number" min={0} max={28} defaultValue={1}/></label><button disabled={busy}>Create disabled schedule</button></form><form className="hospitality-card topology-form" onSubmit={createTrigger}><strong>Internal event trigger</strong><label>Registered event<select name="event"><option value="device_offline">Device offline</option><option value="critical_alert_created">Critical alert created</option><option value="technology_service_degraded">Technology service degraded</option><option value="configuration_drift_detected">Configuration drift detected</option></select></label><button disabled={busy}>Create approval-gated trigger</button></form></div></section>}
    ...
    <section className="hospitality-panel"><div className="panel-heading"><div><h2>Correlation and dead-letter review</h2><span>Grouped triggers remain explainable; failed internal events require administrator review.</span></div></div><div className="hospitality-grid"><article className="hospitality-card"><strong>Correlation groups</strong><span>{correlations.length} recent groups</span><small>{correlations.filter(group=>group.threshold_reached).length} reached threshold</small></article><article className="hospitality-card"><strong>Dead-letter events</strong><span>{deadLetters.length} awaiting review</span><small>Payload content remains hidden</small></article></div></section>
