...
import type { ConsolidatedDiscoveryDevice, IdentityRule, OrganizationDepartment } from "../lib/types";
...
export default function DiscoveryIntelligencePage() {
  ...
  const mode = location.pathname.endsWith("/identity-rules") ? "rules" : location.pathname.endsWith("/devices") ? "devices" : location.pathname.endsWith("/review") ? "review" : "scan";
  ...
  const canApprove=currentUser.data?.role==="admin"||currentUser.data?.role==="platformadmin";
  ...
  const confirmIdentity = async (values:{friendly_name?:string;department?:string;device_type?:string;location?:string}) => {
    const result=(detail?.result||{}) as Record<string,unknown>;if(!result.id)return;
  ...
  const decideIdentity=async(action:"accept"|"reject")=>{const result=(detail?.result||{}) as Record<string,unknown>;if(!result.id)return;setConfirming(true);setError("");try{const updated=action==="accept"?await endpoints.acceptDiscoveryIdentity(String(result.id)):await endpoints.rejectDiscoveryIdentity(String(result.id));setDetail(await endpoints.discoveryResult(String(updated.id)));setDevices(current=>current.map(item=>item.result_id===String(updated.id)?{...item,...updated}:item));setMessage(action==="accept"?"Identity suggestion accepted.":"Identity suggestion rejected; technical identity was preserved.")}catch(caught){setError(caught instanceof Error?caught.message:"Identity review failed.")}finally{setConfirming(false)}};
  ...
  return <DashboardLayout>
    ...
    {mode === "scan" && <>
      <section className="quick-scan-hero"><div><span className="eyebrow">Simple network scan</span><h2>Find devices on your network</h2><p>Enter one private IP address or subnet. No credentials are required.</p></div>{canDiscover?<form onSubmit={scan}><label>Network address or range<input value={range} onChange={(event) => setRange(event.target.value)} placeholder="192.168.1.0/24" required/></label><button className="primary-action" disabled={scanning}>{scanning ? "Scanning…" : "Scan network"}</button></form>:<p>Viewer access is read-only. Discovery actions are unavailable.</p>}{message && <p role="status" className="quick-scan-status">{message}</p>}</section>
    ...
    {mode !== "rules" && (mode !== "scan" || devices.length > 0) && <section className="discovery-panel"><header><div><h2>{mode === "review" ? "Identity review" : "Devices"}</h2><p>{mode === "review" ? "Accept, edit, or reject medium- and low-confidence identity suggestions." : "Discovered devices from your network."}</p></div></header><div className="discovery-filters"><label>Search<input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Name, hostname, IP, MAC, department, type, or vendor"/></label><label>Department<select value={departmentFilter} onChange={event=>setDepartmentFilter(event.target.value)}><option value="">All departments</option>{options(device=>device.department).map(value=><option key={value}>{value}</option>)}</select></label><label>Device type<select value={typeFilter} onChange={event=>setTypeFilter(event.target.value)}><option value="">All device types</option>{options(device=>device.device_type||device.classification).map(value=><option key={value}>{value}</option>)}</select></label><label>Vendor<select value={vendorFilter} onChange={event=>setVendorFilter(event.target.value)}><option value="">All vendors</option>{options(device=>device.vendor).map(value=><option key={value}>{value}</option>)}</select></label></div>{loading ? <Feedback loading/> : <DeviceTable rows={visible} filtered={Boolean(query||departmentFilter||typeFilter||vendorFilter)} inspect={inspect} approve={approve} approving={approving} canApprove={canApprove}/>}</section>}
    {mode !== "rules" && detail && <Evidence data={detail} close={() => setDetail(undefined)} enrich={enrich} enriching={enriching} enrichFromAD={enrichFromAD} adEnriching={adEnriching} confirmIdentity={confirmIdentity} decideIdentity={decideIdentity} confirming={confirming} approve={approveDetail} approving={Boolean(approving)} canDiscover={canDiscover} canApprove={canApprove}/>}
  </DashboardLayout>;
}

const EMPTY_RULE={name:"",scope:"property",match_field:"hostname",match_operator:"contains",pattern:"",output_device_type:"Printer",output_department_id:"",output_location:"",friendly_name_template:"{department} {device_type} {sequence}",priority:100,confidence:70};

function IdentityRulesPanel({canManage}:{canManage:boolean}){
  const [rules,setRules]=useState<IdentityRule[]>([]);const [departments,setDepartments]=useState<OrganizationDepartment[]>([]);const [form,setForm]=useState({...EMPTY_RULE});const [previewInput,setPreviewInput]=useState("");const [preview,setPreview]=useState<Record<string,unknown>>();const [busy,setBusy]=useState(false);const [error,setError]=useState("");const [message,setMessage]=useState("");
  const load=()=>Promise.all([endpoints.identityRules(true),endpoints.organizationDepartments()]).then(([ruleData,departmentData])=>{setRules(ruleData.items);setDepartments(departmentData)}).catch(caught=>setError(caught instanceof Error?caught.message:"Naming rules could not be loaded."));
  useEffect(()=>{void load()},[]);
  const submit=async(event:FormEvent)=>{event.preventDefault();setBusy(true);setError("");try{await endpoints.createIdentityRule({...form,property_id:form.scope==="property"?window.localStorage.getItem("hiop.active_property_id"):null,output_department_id:form.output_department_id||null,output_location:form.output_location||null});setForm({...EMPTY_RULE});setMessage("Naming rule created. It will apply during the next enrichment.");await load()}catch(caught){setError(caught instanceof Error?caught.message:"Rule could not be created.")}finally{setBusy(false)}};
  const test=async()=>{setBusy(true);setError("");try{setPreview(await endpoints.previewIdentityRule({property_id:window.localStorage.getItem("hiop.active_property_id"),hostname:previewInput}))}catch(caught){setError(caught instanceof Error?caught.message:"Rule preview failed.")}finally{setBusy(false)}};
  return <section className="identity-rules-workspace">
    <header className="discovery-panel"><span className="eyebrow">Administration · Discovery intelligence</span><h2>Naming & classification rules</h2><p>Turn hotel-specific technical identifiers into an explainable Friendly Name, Device Type, Suggested Department, and Location. Rules are scoped, deterministic, and never replace a manually confirmed identity.</p><div className="precedence-note"><b>Precedence:</b> Manual confirmation → strong discovery/SNMP → Active Directory → configured rule → weak inference.</div></header>
    {error&&<Feedback error={error}/>} {message&&<p className="quick-scan-status" role="status">{message}</p>}
    <div className="identity-rule-layout"><section className="discovery-panel"><h3>Configured rules</h3>{!rules.length?<Feedback emptyTitle="No naming rules yet." empty="Add a confirmed organization or property naming convention. HIOP will not guess one."/>:<div className="rule-list">{rules.map(rule=><article key={rule.id}><div><strong>{rule.name}</strong><small>{rule.scope} · Priority {rule.priority} · {rule.confidence}% confidence</small></div><p><code>{rule.match_field} {rule.match_operator} “{rule.pattern}”</code></p><p>{rule.department_name||"No department"} · {rule.output_device_type||"No device type"} · {rule.output_location||"No location"}</p><StatusBadge status={rule.enabled?"Active":"Inactive"}/>{canManage&&rule.enabled&&<button onClick={()=>void endpoints.disableIdentityRule(rule.id).then(load)}>Disable</button>}</article>)}</div>}</section>
      <section className="discovery-panel">{canManage?<form className="identity-rule-form" onSubmit={submit}><h3>Add rule</h3><label>Rule name<input required value={form.name} onChange={event=>setForm({...form,name:event.target.value})} placeholder="Front Office printer hostname"/></label><div className="form-grid"><label>Scope<select value={form.scope} onChange={event=>setForm({...form,scope:event.target.value})}><option value="property">Current property</option><option value="organization">Organization-wide</option></select></label><label>Evidence field<select value={form.match_field} onChange={event=>setForm({...form,match_field:event.target.value})}><option value="hostname">Hostname</option><option value="fqdn">FQDN</option><option value="vendor">Vendor</option><option value="snmp">SNMP description</option><option value="ad_ou">Active Directory OU</option><option value="device_type">Existing device type</option></select></label><label>Match<select value={form.match_operator} onChange={event=>setForm({...form,match_operator:event.target.value})}><option value="contains">Contains</option><option value="starts_with">Starts with</option><option value="ends_with">Ends with</option><option value="equals">Equals</option></select></label><label>Pattern<input required value={form.pattern} onChange={event=>setForm({...form,pattern:event.target.value})} placeholder="Confirmed token"/></label><label>Device type<select value={form.output_device_type} onChange={event=>setForm({...form,output_device_type:event.target.value})}>{["Printer","POS Terminal","Desktop","Laptop","Server","Switch","Router","Firewall","Access Point","UPS","Phone","Other"].map(value=><option key={value}>{value}</option>)}</select></label><label>Department<select value={form.output_department_id} onChange={event=>setForm({...form,output_department_id:event.target.value})}><option value="">No department</option>{departments.filter(item=>item.is_active).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Location (optional)<input value={form.output_location} onChange={event=>setForm({...form,output_location:event.target.value})} placeholder="Only with reliable evidence"/></label><label>Confidence<input type="number" min="1" max="100" value={form.confidence} onChange={event=>setForm({...form,confidence:Number(event.target.value)})}/></label></div><label>Friendly name template<input value={form.friendly_name_template} onChange={event=>setForm({...form,friendly_name_template:event.target.value})}/><small>Available: department, device_type, location, sequence, hostname.</small></label><button className="primary-action" disabled={busy}>{busy?"Saving…":"Add rule"}</button></form>:<p>You can inspect rules. Organization administrators manage them.</p>}
        <div className="rule-preview"><h3>Preview safely</h3><p>Preview does not change a device.</p><label>Technical hostname<input value={previewInput} onChange={event=>setPreviewInput(event.target.value)} placeholder="heloshadtsc1821"/></label><button disabled={busy||!previewInput} onClick={()=>void test()}>Test rules</button>{preview&&<pre>{JSON.stringify(preview,null,2)}</pre>}</div></section></div>
  </section>
}
...
function DeviceTable({ rows, filtered, inspect, approve, approving, canApprove }: { rows: ConsolidatedDiscoveryDevice[]; filtered:boolean; inspect: (device: ConsolidatedDiscoveryDevice) => void; approve: (device: ConsolidatedDiscoveryDevice) => void; approving: string; canApprove:boolean }) {
...
function Evidence({ data, close, enrich, enriching, enrichFromAD, adEnriching, confirmIdentity, decideIdentity, confirming, approve, approving, canDiscover, canApprove }: { data: Record<string, unknown>; close: () => void; enrich:()=>void; enriching:boolean; enrichFromAD:()=>void; adEnriching:boolean; confirmIdentity:(values:{friendly_name?:string;department?:string;device_type?:string;location?:string})=>void; decideIdentity:(action:"accept"|"reject")=>void; confirming:boolean; approve:()=>Promise<void>; approving:boolean; canDiscover:boolean; canApprove:boolean }) {
  ...
  const [friendlyName,setFriendlyName]=useState(String(result.friendly_name||""));const [department,setDepartment]=useState(String(result.department||""));const [deviceType,setDeviceType]=useState(String(result.device_type||result.classification||""));const [location,setLocation]=useState(String(result.location||""));
  ...
  return <section className="discovery-detail"><header><div><small>Why HIOP identified this device</small><h3>{String(result.friendly_name || result.primary_hostname || result.ip_address || "Unable to identify this device")}</h3></div><div className="row-actions">{canDiscover&&<><button className="primary-action" disabled={enriching} onClick={()=>void enrich()}>{enriching?"Enriching…":"Enrich Device"}</button><button disabled={adEnriching} onClick={()=>void enrichFromAD()}>{adEnriching?"Checking AD…":"Enrich from Active Directory"}</button></>}<button onClick={close}>Close</button></div></header>
    ...
    <div className="detail-grid"><article><h4>Identity</h4><p><b>Friendly Name:</b> {String(result.friendly_name || "Unknown Device")}</p><p><b>Name source:</b> {String(profile.friendly_name_source || (result.identity_confirmed?"MANUAL":"Not yet classified"))}</p><p><b>Original Hostname:</b> {String(result.primary_hostname || "Not available")}</p><p><b>FQDN:</b> {String(result.fqdn || "Not available")}</p><p><b>IP:</b> {String(result.ip_address)}</p><p><b>MAC:</b> {String(result.mac_address || "Not available")}</p></article><article><h4>Classification</h4><p><b>Device Type:</b> {String(result.device_type || result.classification || "Unknown")}</p><p><b>Type source:</b> {String(profile.classification_source||(result.identity_confirmed?"MANUAL":"Discovery"))}</p>{Boolean(result.classification)&&result.classification!==result.device_type&&<p><b>Technical Classification:</b> {String(result.classification)}</p>}<p><b>Department:</b> {String(result.department || "Unknown")}</p><p><b>Department source:</b> {String(profile.department_source||(result.identity_confirmed?"MANUAL":"Not yet classified"))}</p><p><b>Location:</b> {String(result.location || "Unknown")}</p><p><b>Location source:</b> {String(profile.location_source||(result.identity_confirmed?"MANUAL":"Not yet discovered"))}</p></article><article><h4>Technical</h4><p><b>Vendor:</b> {String(result.vendor || "Unknown")}</p><p><b>Model:</b> {String(result.model || "Not available")}</p><p><b>Serial Number:</b> {String(result.serial_number || "Not available")}</p><p><b>Operating System:</b> {String(result.operating_system||result.ad_operating_system||"Not available")}</p><p><b>Firmware:</b> {String(result.firmware || "Not available")}</p><p><b>Uptime:</b> {uptime}</p><p><b>Interfaces:</b> {String(result.interface_count??"Not available")}</p>{interfaces.slice(0,8).map((item,index)=><small key={index}>{String(item.name||item.description||`Interface ${item.index}`)} — {String(item.oper_status||"Unknown")}</small>)}</article><article><h4>Windows / Active Directory</h4><p><b>Domain:</b> {String(result.ad_domain||"Not available")}</p><p><b>Computer:</b> {String(result.ad_computer_name||"Not available")}</p><p><b>OS Version:</b> {String(result.ad_operating_system_version||"Not available")}</p><p><b>OU:</b> {String(result.ad_organizational_unit||"Not available")}</p><p><b>AD Status:</b> {result.ad_enabled===true?"Enabled":result.ad_enabled===false?"Disabled":"Not available"}</p><p><b>Last Logon:</b> {result.ad_last_logon_at?new Date(String(result.ad_last_logon_at)).toLocaleString():"Not available"}</p><p><b>Description:</b> {String(result.ad_description||result.description||"Not available")}</p><p><b>Distinguished Name:</b> {String(result.ad_distinguished_name||"Not available")}</p></article></div>
    ...
    {canDiscover&&!result.identity_confirmed&&typeof result.friendly_name==="string"&&result.friendly_name.length>0&&<div className="identity-review-actions"><button className="primary-action" disabled={confirming} onClick={()=>decideIdentity("accept")}>Accept suggestion</button><button disabled={confirming} onClick={()=>decideIdentity("reject")}>Reject suggestion</button></div>}
    {canDiscover&&<form className="identity-confirmation" onSubmit={event=>{event.preventDefault();confirmIdentity({friendly_name:friendlyName,department,device_type:deviceType,location})}}><h4>Manual correction</h4><p>Confirm or correct the suggested identity. Manually confirmed values are preserved during future discovery.</p><label>Friendly Name<input value={friendlyName} onChange={event=>setFriendlyName(event.target.value)}/></label><label>Department<input value={department} onChange={event=>setDepartment(event.target.value)}/></label><label>Device Type<input value={deviceType} onChange={event=>setDeviceType(event.target.value)}/></label><label>Location<input value={location} onChange={event=>setLocation(event.target.value)}/></label><button className="primary-action" disabled={confirming}>{confirming?"Confirming…":"Confirm Identification"}</button></form>}
    ...
    <footer className="identity-review-actions">{approved?<><StatusBadge status="Approved"/><span>This device exists once in managed inventory.</span></>:strong&&canApprove?<button className="primary-action" disabled={approving} onClick={()=>void approve()}>{approving?"Approving…":"Approve Device"}</button>:<><StatusBadge status="Review"/><span>{canApprove?"Review conflicts or confirm the identity before approval.":"Read-only evidence view."}</span></>}</footer>
  </section>;
}
