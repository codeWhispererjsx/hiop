...
function Evidence({ data, close }: { data: Record<string, unknown>; close: () => void }) {
  ...
  return <section className="discovery-detail"><header><div><small>Why HIOP identified this device</small><h3>{String(result.friendly_name || result.primary_hostname || result.ip_address || "Device evidence")}</h3></div><button onClick={close}>Close</button></header><div className="detail-grid"><article><h4>Identity</h4><p><b>Friendly Name:</b> {String(result.friendly_name || "Unknown")}</p><p><b>Hostname:</b> {String(result.primary_hostname || "—")}</p><p><b>FQDN:</b> {String(result.fqdn || "—")}</p><p><b>IP:</b> {String(result.ip_address)}</p><p><b>MAC:</b> {String(result.mac_address || "—")}</p><p><b>Description:</b> {String(result.description || "Not available")} {result.description_source ? `(${String(result.description_source)})` : ""}</p></article><article><h4>Classification</h4><p><b>Device Type:</b> {String(result.classification || result.device_type || "Unknown")}</p><p><b>Department:</b> {String(result.department || "—")}</p><p><b>Location:</b> {String(result.location || "—")}</p><p><b>Original Hostname:</b> {String(result.primary_hostname || "—")}</p><p><b>DNS status:</b> {String(result.dns_status || "not attempted").replaceAll("_", " ")}</p></article></div><h4>Evidence</h4><div className="evidence-cards">{evidence.length ? evidence.map((item, index) => <article key={String(item.id || index)}><span>+{String(item.weight || 0)}%</span><strong>{String(item.evidence_type || "evidence").replaceAll("_", " ")}</strong><small>{String(item.source || "network")}</small></article>) : <p>Not available</p>}</div></section>;
}
