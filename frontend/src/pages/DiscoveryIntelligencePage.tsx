...
export default function DiscoveryIntelligencePage() {
  ...
  return <DashboardLayout>
    ...
    {detail && <Evidence data={detail} close={() => setDetail(undefined)} enrich={enrich} enriching={enriching}/>}
  </DashboardLayout>;
}
...
function Evidence({ data, close, enrich, enriching }: { data: Record<string, unknown>; close: () => void; enrich:()=>void; enriching:boolean }) {
  ...
  return <section className="discovery-detail"><header><div><small>Why HIOP identified this device</small><h3>{String(result.friendly_name || result.primary_hostname || result.ip_address || "Device evidence")}</h3></div><div className="row-actions"><button className="primary-action" disabled={enriching} onClick={()=>void enrich()}>{enriching?"Enriching…":"Enrich Device"}</button><button onClick={close}>Close</button></div></header><p className="enrichment-status"><b>SNMP:</b> {String(result.snmp_enrichment_status||"not_attempted").replaceAll("_"," ")}</p><div className="detail-grid"><article><h4>Device identity</h4><p><b>Friendly Name:</b> {String(result.friendly_name || "Unknown")}</p><p><b>Hostname:</b> {String(result.primary_hostname || "—")}</p><p><b>FQDN:</b> {String(result.fqdn || "—")}</p><p><b>IP:</b> {String(result.ip_address)}</p><p><b>MAC:</b> {String(result.mac_address || "—")}</p></article><article><h4>Classification</h4><p><b>Device Type:</b> {String(result.classification || result.device_type || "Unknown")}</p><p><b>Department:</b> {String(result.department || "—")}</p><p><b>Vendor:</b> {String(result.vendor || "Unknown")}</p><p><b>Model:</b> {String(result.model || "Not available")}</p></article><article><h4>Technical</h4><p><b>Serial Number:</b> {String(result.serial_number || "Not available")}</p><p><b>Firmware:</b> {String(result.firmware || "Not available")}</p><p><b>Uptime:</b> {uptime}</p><p><b>sysObjectID:</b> {String(result.sys_object_id || "Not available")}</p><p><b>Interfaces:</b> {String(result.interface_count??"Not available")}</p>{interfaces.slice(0,8).map((item,index)=><small key={index}>{String(item.name||item.description||`Interface ${item.index}`)} — {String(item.oper_status||"Unknown")}</small>)}</article></div><h4>Evidence</h4><div className="evidence-cards">{evidence.length ? evidence.map((item, index) => <article key={String(item.id || index)}><span>+{String(item.weight || 0)}%</span><strong>{String(item.evidence_type || "evidence").replaceAll("_", " ")}</strong><small>{String(item.source || "network")}</small></article>) : <p>Not available</p>}</div></section>;
}
