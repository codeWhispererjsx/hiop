...
export default function DiscoveryIntelligencePage() {
  ...
  const canApprove=currentUser.data?.role==="admin";
  ...
  return <DashboardLayout>
    ...
    {detail && <Evidence data={detail} close={() => setDetail(undefined)} enrich={enrich} enriching={enriching} enrichFromAD={enrichFromAD} adEnriching={adEnriching} confirmIdentity={confirmIdentity} confirming={confirming} approve={approveDetail} approving={Boolean(approving)} canDiscover={canDiscover} canApprove={canApprove}/>}
  </DashboardLayout>;
}
...
function Evidence({ data, close, enrich, enriching, enrichFromAD, adEnriching, confirmIdentity, confirming, approve, approving, canDiscover, canApprove }: { data: Record<string, unknown>; close: () => void; enrich:()=>void; enriching:boolean; enrichFromAD:()=>void; adEnriching:boolean; confirmIdentity:(values:{friendly_name?:string;department?:string;device_type?:string})=>void; confirming:boolean; approve:()=>Promise<void>; approving:boolean; canDiscover:boolean; canApprove:boolean }) {
  ...
  return <section className="discovery-detail"><header><div><small>Why HIOP identified this device</small><h3>{String(result.friendly_name || result.primary_hostname || result.ip_address || "Unable to identify this device")}</h3></div><div className="row-actions">{canDiscover&&<><button className="primary-action" disabled={enriching} onClick={()=>void enrich()}>{enriching?"Enriching…":"Enrich Device"}</button><button disabled={adEnriching} onClick={()=>void enrichFromAD()}>{adEnriching?"Checking AD…":"Enrich from Active Directory"}</button></>}<button onClick={close}>Close</button></div></header>
    ...
    {canDiscover&&<form className="identity-confirmation" onSubmit={event=>{event.preventDefault();confirmIdentity({friendly_name:friendlyName,department,device_type:deviceType})}}><h4>Manual correction</h4><p>Confirm or correct the suggested identity. Manually confirmed values are preserved during future discovery.</p><label>Friendly Name<input value={friendlyName} onChange={event=>setFriendlyName(event.target.value)}/></label><label>Department<input value={department} onChange={event=>setDepartment(event.target.value)}/></label><label>Device Type<input value={deviceType} onChange={event=>setDeviceType(event.target.value)}/></label><button className="primary-action" disabled={confirming}>{confirming?"Confirming…":"Confirm Identification"}</button></form>}
