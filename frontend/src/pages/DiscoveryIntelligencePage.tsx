...
export default function DiscoveryIntelligencePage() {
  ...
  return <DashboardLayout>
    ...
    {detail && <Evidence data={detail} close={() => setDetail(undefined)} enrich={enrich} enriching={enriching} enrichFromAD={enrichFromAD} adEnriching={adEnriching} confirmIdentity={confirmIdentity} confirming={confirming}/>}
  </DashboardLayout>;
}
...
function Evidence({ data, close, enrich, enriching, enrichFromAD, adEnriching, confirmIdentity, confirming }: { data: Record<string, unknown>; close: () => void; enrich:()=>void; enriching:boolean; enrichFromAD:()=>void; adEnriching:boolean; confirmIdentity:(values:{friendly_name?:string;department?:string;device_type?:string})=>void; confirming:boolean }) {
  ...
  const [friendlyName,setFriendlyName]=useState(String(result.friendly_name||""));const [department,setDepartment]=useState(String(result.department||""));const [deviceType,setDeviceType]=useState(String(result.classification||result.device_type||""));
  ...
  return <section className="discovery-detail"><header><div><small>Why HIOP identified this device</small><h3>{String(result.friendly_name || result.primary_hostname || result.ip_address || "Device evidence")}</h3></div><div className="row-actions"><button className="primary-action" disabled={enriching} onClick={()=>void enrich()}>{enriching?"Enriching…":"Enrich Device"}</button><button disabled={adEnriching} onClick={()=>void enrichFromAD()}>{adEnriching?"Checking AD…":"Enrich from Active Directory"}</button><button onClick={close}>Close</button></div></header><div className="enrichment-statuses"><p className="enrichment-status"><b>SNMP:</b> {String(result.snmp_enrichment_status||"not_attempted").replaceAll("_"," ")}</p><p className="enrichment-status"><b>Active Directory:</b> {String(result.ad_enrichment_status||"not_attempted").replaceAll("_"," ")}</p></div><div className="detail-grid"><article><h4>Device identity</h4><p><b>Friendly Name:</b> {String(result.friendly_name || "Unknown")}</p><p><b>Hostname:</b> {String(result.primary_hostname || "—")}</p><p><b>FQDN:</b> {String(result.fqdn || "—")}</p><p><b>IP:</b> {String(result.ip_address)}</p><p><b>MAC:</b> {String(result.mac_address || "—")}</p></article><article><h4>Classification</h4><p><b>Device Type:</b> {String(result.classification || result.device_type || "Unknown")}</p><p><b>Department:</b> {String(result.department || "—")}</p><p><b>Suggested Department:</b> {String(result.suggested_department || "Not available")}</p><p><b>Vendor:</b> {String(result.vendor || "Unknown")}</p><p><b>Model:</b> {String(result.model || "Not available")}</p></article><article><h4>Enrichment</h4><p><b>Serial Number:</b> {String(result.serial_number || "Not available")}</p><p><b>Operating System:</b> {String(result.operating_system||result.ad_operating_system||"Not available")}</p><p><b>Firmware:</b> {String(result.firmware || "Not available")}</p><p><b>Uptime:</b> {uptime}</p><p><b>Interfaces:</b> {String(result.interface_count??"Not available")}</p>{interfaces.slice(0,8).map((item,index)=><small key={index}>{String(item.name||item.description||`Interface ${item.index}`)} — {String(item.oper_status||"Unknown")}</small>)}</article><article><h4>Windows / Active Directory</h4><p><b>Domain:</b> {String(result.ad_domain||"Not available")}</p><p><b>Computer:</b> {String(result.ad_computer_name||"Not available")}</p><p><b>OS Version:</b> {String(result.ad_operating_system_version||"Not available")}</p><p><b>OU:</b> {String(result.ad_organizational_unit||"Not available")}</p><p><b>AD Status:</b> {result.ad_enabled===true?"Enabled":result.ad_enabled===false?"Disabled":"Not available"}</p><p><b>Last Logon:</b> {result.ad_last_logon_at?new Date(String(result.ad_last_logon_at)).toLocaleString():"Not available"}</p><p><b>Description:</b> {String(result.ad_description||"Not available")}</p><p><b>Distinguished Name:</b> {String(result.ad_distinguished_name||"Not available")}</p></article></div><article className="identity-confidence"><h4>Confidence</h4><strong>{String(result.confidence_score||0)}% · {String(result.confidence_level||"low").replaceAll("_"," ")}</strong><p>{String(result.confidence_reason||"Insufficient evidence.")}</p><small>{result.identity_confirmed?"Identity manually confirmed":"Identity not manually confirmed"} · {String(data.correlated_observations||1)} correlated observation(s)</small></article>{conflicts.length>0&&<section className="identity-conflicts"><h4>Conflicts</h4>{conflicts.map((item,index)=><article key={String(item.id||index)}><StatusBadge status={String(item.status||"open")}/><strong>{String(item.attribute)}</strong><p>{String(item.left_source)}: {String(item.left_value)} ↔ {String(item.right_source)}: {String(item.right_value)}</p></article>)}</section>}<form className="identity-confirmation" onSubmit={event=>{event.preventDefault();confirmIdentity({friendly_name:friendlyName,department,device_type:deviceType})}}><h4>Confirm identification</h4><label>Friendly Name<input value={friendlyName} onChange={event=>setFriendlyName(event.target.value)}/></label><label>Department<input value={department} onChange={event=>setDepartment(event.target.value)}/></label><label>Device Type<input value={deviceType} onChange={event=>setDeviceType(event.target.value)}/></label><button className="primary-action" disabled={confirming}>{confirming?"Confirming…":"Confirm Identification"}</button></form><h4>Evidence</h4><div className="evidence-cards">{evidence.length ? evidence.map((item, index) => <article key={String(item.id || index)}><span>+{String(item.weight || 0)}%</span><strong>{String(item.evidence_type || "evidence").replaceAll("_", " ")}</strong><small>{String(item.source || "network")} · {item.verified?"confirmed":"observed"}</small></article>) : <p>Not available</p>}</div>{history.length>0&&<section className="identity-history"><h4>Identity history</h4>{history.map((item,index)=><p key={String(item.id||index)}><b>{String(item.attribute)}</b>: {String(item.previous_value||"Unknown")} → {String(item.current_value)} <small>{String(item.source)}</small></p>)}</section>}</section>;
}
