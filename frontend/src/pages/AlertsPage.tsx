...
const alertSeverity = (alert: Alert): Severity =>
  alert.current_status === "Offline" ? "Critical" : "Informational";
const alertState = (alert: Alert): AlertState =>
...
export default function AlertsPage() {
  ...
  const ticketByDevice = useMemo(() => {
    ...
    for (const ticket of tickets.data ?? [])
      if (ticket.device_id && !map.has(ticket.device_id))
        map.set(ticket.device_id, ticket);
    return map;
  }, [tickets.data]);
  const departments = useMemo(
    () =>
      [
        ...new Set(
          (devices.data ?? [])
            .map((device) => device.department.trim())
            .filter(Boolean),
        ),
      ].sort(),
    [devices.data],
  ...
  const rows = useMemo(
    () =>
      (alerts.data ?? []).filter((alert) => {
        ...
        const text =
          `${alert.message} ${alert.previous_status} ${alert.current_status} ${device?.hostname ?? ""} ${device?.asset_tag ?? ""} ${device?.department ?? ""}`.toLowerCase();
        return (
          (!deferredSearch.trim() ||
  ...
  const acknowledge = async (alert: Alert) => {
    ...
    try {
      ...
      await reloadAlerts();
      setSelected((current) =>
        current?.id === alert.id ? { ...current, acknowledged: true } : current,
      );
    } catch (error) {
      notify(
  ...
  const live = useCallback(
    (event: LiveEvent) => {
      if (event.event === "device_status_changed") {
        setToast({
          id: Date.now(),
          message: `${event.hostname ?? "A device"} changed to ${event.current_status ?? "a new state"}.`,
          tone: "success",
        });
        void reloadAlerts();
      }
  ...
  const handleLiveState = useCallback((connected: boolean) => {
    setSocketState(
      connected ? "connected" : navigator.onLine ? "reconnecting" : "offline",
    );
  }, []);
  const clearFilters = () => {
    setSearch("");
  ...
  return (
    <DashboardLayout onLiveEvent={live} onLiveStateChange={handleLiveState}>
      <PageTitle
        eyebrow="Enterprise response centre"
        title="Alerts management"
        copy="Triage persistent infrastructure events, correlate affected assets and coordinate operational response."
        action={
        ...
        <>
          <section className="alerts-summary">
            <StatCard
              ...
              detail="All persisted monitoring events"
              icon="alerts"
              trend="Recorded"
            />
            <StatCard
              label="Active alerts"
              value={allAlerts.filter((item) => !item.acknowledged).length}
            ...
            <StatCard
              ...
              value="—"
              detail="Resolution is not yet tracked by the API"
              icon="check"
              trend="API gap"
            />
            <StatCard
              ...
              value={
                allAlerts.filter((item) => alertSeverity(item) === "Critical")
                  .length
              }
              detail="Current status is Offline"
              icon="warning"
              tone="danger"
              trend="Critical"
            />
            <StatCard
              ...
              value={
                allAlerts.filter(
                  (item) => dayValue(item.created_at) === todayValue(),
                ).length
              }
              detail="Created in the local calendar day"
              icon="clock"
              trend="Today"
            />
          </section>
          ...
          <section className="toolbar-panel alerts-toolbar">
            <label className="search-field">
              ...
              <input
                ...
                placeholder="Search alerts, devices or asset tags"
                value={search}
            ...
            <input
              className="alerts-date"
              aria-label="Alert date"
              type="date"
              value={date}
              onChange={(event) => setDate(event.target.value)}
            />
            <button className="secondary-action" onClick={clearFilters}>
              Clear
            </button>
          </section>
          ...
          <section className="panel alerts-table-panel">
            <header className="section-head">
              <div>
                <h2>Alert queue</h2>
                <p>
                  {rows.length} of {allAlerts.length} alerts match the current
                  view.
                </p>
              </div>
            </header>
            {!rows.length ? (
              <Feedback
                ...
                empty="No alerts match the selected filters."
              />
            ) : (
              <AlertsTable
                alerts={rows}
                ...
                onDevice={(id) => navigate(`/devices/${id}`)}
              />
            )}
          </section>
        </>
      ...
      {selected && (
        <AlertDetailsPanel
          key={selected.id}
          alert={selected}
          device={deviceMap.get(selected.device_id)}
          ticket={ticketByDevice.get(selected.device_id)}
          busy={busy === selected.id}
          onClose={() => setSelected(null)}
          onAcknowledge={() => void acknowledge(selected)}
          onDevice={() => navigate(`/devices/${selected.device_id}`)}
        ...
        <tbody>
          {alerts.map((alert) => {
            ...
            return (
              <tr
                key={alert.id}
                className={!alert.acknowledged ? "active-alert" : ""}
              >
                ...
                <td>Device status change</td>
                <td>
                  <span
                    className={`severity-badge ${alertSeverity(alert).toLowerCase()}`}
                  >
                    {alertSeverity(alert)}
                  </span>
                </td>
                <td>
                  <StatusBadge status={alert.previous_status} />
                </td>
                <td>
                ...
                <td>Not linked by API</td>
                <td>
                  <span
                    className={`alert-state ${alert.acknowledged ? "acknowledged" : "active"}`}
                  >
                    {alertState(alert)}
                  </span>
                </td>
                <td>
                  <div className="row-actions">
                    <button onClick={() => onSelect(alert)}>Details</button>
                    {!alert.acknowledged && (
                      <button
