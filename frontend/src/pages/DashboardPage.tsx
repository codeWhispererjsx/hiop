    ...
    {dashboard.loading || inventory.loading || dashboard.error || inventory.error || !d
      ...
      : <>
        <section className="asset-summary-grid">
          ...
          <article className="asset-metric-card"><span>Active alerts</span><strong>{(alerts.data?.items??[]).filter(item=>item.status!=="resolved").length}</strong><small>Open or acknowledged alerts</small></article>
