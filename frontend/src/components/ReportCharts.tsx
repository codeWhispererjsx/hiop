...
const COLORS = ["var(--color-primary)", "var(--color-success)", "var(--color-danger)", "var(--color-info)", "var(--color-maintenance)", "var(--color-offline)"];
...
export function TrendChart({ title, data }: { title: string; data: Array<{ name: string; value?: number; total?: number }> }) {
  ...
  return <article className="report-chart-card report-trend-card"><header><h3>{title}</h3><span>{data.length} days</span></header>{data.length ? <><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${title} trend`}><defs><linearGradient id={`trend-${title.replaceAll(" ", "-")}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="var(--color-primary)" stopOpacity=".28"/><stop offset="1" stopColor="var(--color-primary)" stopOpacity="0"/></linearGradient></defs><polyline className="report-trend-line" points={points}/></svg><div className="report-trend-axis"><span>{data[0]?.name}</span><span>{data[data.length - 1]?.name}</span></div></> : <p className="report-chart-empty">Historical data is unavailable for this period.</p>}</article>;
}
