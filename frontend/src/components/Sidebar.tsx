...
const links: { label: string; to: string; icon: IconName }[] = [
  { label: "Overview", to: "/dashboard", icon: "dashboard" }, { label: "Reports", to: "/reports", icon: "audit" }, { label: "Devices", to: "/devices", icon: "devices" }, { label: "Network monitor", to: "/network", icon: "network" }, { label: "Discovery", to: "/discovery", icon: "discovery" }, { label: "Alerts", to: "/alerts", icon: "alerts" }, { label: "Service tickets", to: "/tickets", icon: "tickets" }, { label: "Locations & structure", to: "/hierarchy", icon: "hierarchy" }, { label: "Team & access", to: "/users", icon: "users" }, { label: "Audit trail", to: "/audit", icon: "audit" }, { label: "Settings", to: "/settings", icon: "settings" },
];

export default function Sidebar({ open, onClose, role }: { open: boolean; onClose: () => void; role?: string }) {
  const visibleLinks = role === "admin" ? links : links.filter(link => !["/reports", "/users", "/audit", "/settings"].includes(link.to));
  return <aside className={`sidebar ${open ? "is-open" : ""}`}>
    ...
    <nav className="sidebar-nav" aria-label="Primary navigation">
      ...
      {visibleLinks.filter(link => links.indexOf(link) < 7).map(link => <NavLink key={link.to} to={link.to} onClick={onClose} className={({isActive}) => `nav-link ${isActive ? "active" : ""}`}><Icon name={link.icon}/><span>{link.label}</span></NavLink>)}
