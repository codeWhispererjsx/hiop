...
export default function Sidebar({ open, onClose, role }: { open: boolean; onClose: () => void; role?: string }) {
  ...
  return <aside className={`sidebar ${open ? "is-open" : ""}`}>
    ...
    <nav className="sidebar-nav" aria-label="Primary navigation">
      ...
      {visibleLinks.filter(link => links.indexOf(link) < 6).map(link => <NavLink key={link.to} to={link.to} onClick={onClose} className={({isActive}) => `nav-link ${isActive ? "active" : ""}`}><Icon name={link.icon}/><span>{link.label}</span></NavLink>)}
