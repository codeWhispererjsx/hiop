...
export default function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  return <aside className={`sidebar ${open ? "is-open" : ""}`}>
    ...
    <nav className="sidebar-nav" aria-label="Primary navigation">
      ...
      {links.slice(0, 5).map(link => <NavLink key={link.to} to={link.to} onClick={onClose} className={({isActive}) => `nav-link ${isActive ? "active" : ""}`}><Icon name={link.icon}/><span>{link.label}</span></NavLink>)}
      <p className="nav-label nav-label-spaced">Administration</p>
      {links.slice(5).map(link => <NavLink key={link.to} to={link.to} onClick={onClose} className={({isActive}) => `nav-link ${isActive ? "active" : ""}`}><Icon name={link.icon}/><span>{link.label}</span></NavLink>)}
    </nav>
