...
Epic 1D adds hospitality asset classification foundations, property technology services, operations summary APIs, and a property operations page via migration `c3d4e5f6a7b8`.
...
Epic 3B core orchestration now includes fixed-catalogue internal events, property-scoped trigger subscriptions, structured filters/conditions/input mappings, replay protection and reviewed reprocessing, correlation-window deduplication/cooldown/storm controls, delayed and approval-gated execution, interval and one-time schedules, maintenance/blackout suppression, stable APScheduler jobs, overlap prevention, startup reconciliation, stale-run recovery, audit/live updates, scheduler monitoring, Automation workspace controls, and additive migration `f4b5c6d7e8f9`. Full Epic 3B completion still requires persistent outbox/dead-letter processing, multi-event correlation groups, recovery-event cancellation, calendar recurrence, retention jobs/reports, and their complete UI/test coverage. Public webhooks, arbitrary event types, arbitrary scripts, and unreviewed destructive actions remain prohibited.
